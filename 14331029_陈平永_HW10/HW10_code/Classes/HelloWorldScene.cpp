...
bool HelloWorld::init()
{
	...
	//倒计时
	CCString* ccTime = CCString::createWithFormat("%d", dtime);
	time = Label::createWithTTF(ttfConfig, ccTime->getCString());
...
void HelloWorld::moveEvent(Ref*,char cid) {
	...
	case 'W':       //向上移动
		if (playerY < 440) {
			player->runAction(MoveTo::create(0.5, 
				Vec2(playerX, playerY + 40)));
		}
		break;
	case 'A':       //向左移动
		if (playerX > 41) {
			player->runAction(MoveTo::create(0.5,
				Vec2(playerX - 40, playerY)));
		}
		break;
	case 'S':       //向下移动
		if (playerY > 40) {
...
void HelloWorld::actionEvent(Ref*, char cid) {
	...
	animation = Animation::createWithSpriteFrames(style, 0.1f);
	auto animate = Animate::create(animation);
	player->runAction(Repeat::create(animate->clone(), 1));
}
